...
$uppayment=get_option('userpro_payment');
			...
			case 'register':
				...
				$user_invited = register_invited_user_only($user_email);
	...
	function userpro_user_invite() {
				delete_option('userpro_invited_users');
	...
	function userpro_invite_mail_send($users){
		...
		foreach ($users as $user) {
		   if( email_exists( $user ) == false) {
			...
			$message .= preg_replace('/{inivitelink}/', $invite_link.'?code='.urlencode($crypted), $invite_email_template);
			$message .= '</body></html>';
			$headers = "MIME-Version: 1.0\r\n";
			$headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
			mail($user,'You are invited to register at '.get_bloginfo('name'), html_entity_decode($message)  , $headers);
