...
if (!in_array($sortby, $defaultorder))
		...
		$query="SELECT a.ID
			FROM wp_users AS a
			JOIN wp_usermeta ON a.ID= wp_usermeta.user_id WHERE wp_usermeta.meta_key = '$sortby' AND wp_usermeta.user_id = a.ID ORDER BY   wp_usermeta.meta_value $order";
		...
		$roles = explode(',',$role);
		if(isset($search_string) && $search_string!="" && count($roles) >= 2)
		{
			...
			if($key=="users")
			{
				...
				foreach($value as $user)
				{
					...
					foreach($user->roles as $role)
					{
						...
						{       
							if($count==1 &&  stripos($user->data->display_name,$search_string) !== false)
							{
								$user_list=array_merge($user_list,array($user));
								$count++;
							}
						}
					}
