...
function linkedin_authorize(){
		if(isset($_REQUEST['wplUsername'])){
			...
			if(!$user_id){
				...
				if(!$user_id){
					...
					$this->instagram_save_profile_pic($user_id,$profileurl);
				}
			}
	...
	function instagram_authorize(){
		if(isset($_REQUEST['wpInDisplayName'])){
			...
			if(!$user_id){
					...
					$this->instagram_save_profile_pic($user_id,$profilePic);
			}
	...
	******************************************/
	function get_member_by( $arg, $force=0 ) {
		if ($force) {
			...
			if ($nice_url == 'username') {
				/*commented by Yogesh for allowed dash in username*/
				//$arg = str_replace('-',' ', $arg);
				$user = get_user_by('login', $arg);
			}
