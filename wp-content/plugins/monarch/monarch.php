...
class ET_Monarch {
				...
				foreach ( $option['value'] as $network => $network_name ) {
					...
					$result .= sprintf(
						...
						( 'soundcloud' == $network || 'dribbble' == $network )
							...
							: '',
						( 'vkontakte' == $network || 'facebook' == $network || 'github' == $network )
							? sprintf( ' data-client_id_placeholder="%1$s"',
								'vkontakte' == $network
	...
	function db_install() {
		...
		$db_version = array( 'db_version' => $this->db_version );
		$this->update_option( $db_version );
	}
	...
	*/
																	...
																	for( $i = 0; $i < $networks_count; $i++ ) {
																		...
																		printf( '<div class="et_social_network et_social_icon ui-sortable-handle%8$s%10$s" data-name="%1$s" data-area="%2$s"><span class="et_social_%1$s" ><a href="#" class="et_social_deletenetwork"></a></span><input type="text" class="input_label" placeholder="%1$s" name="et_social[%2$s][label][%3$s]" value="%4$s">%6$s%9$s%7$s%5$s<input class="input_class" type="hidden" name="et_social[%2$s][class][%3$s]" value="%1$s" /></div>',
																				...
																				( ( 'twitter' == $network_class && 'follow' != $current_section ) || ( 'follow' == $current_section && 'like' != $network_class ) )
																					...
																					: '',
																				( 'follow' == $current_section && ( 'vkontakte' == $network_class || 'facebook' == $network_class || 'soundcloud' == $network_class || 'dribbble' == $network_class || 'github' == $network_class ) )
																				...
																				: '',
																				( 'follow' == $current_section && ( 'vkontakte' == $network_class || 'facebook' == $network_class || 'dribbble' == $network_class || 'github' == $network_class ) ) ? ' et_social_4_fields' : '',
	...
	function api_generate_authorization_url() {
		...
		if ( '' !== $authorization_url ) {
			if ( isset( $_POST['options'] ) && empty( $error_messages ) ) {
	...
	*/
	...
	function get_share_link( $network, $media_url = '', $i = 0 ) {
		if ( '' !== $network ) {
			$monarch_options = $this->monarch_options;
			...
			switch ( $network ) {
				...
				case 'twitter' :
					...
					$link = sprintf( 'http://twitter.com/home?status=%2$s%3$s%1$s%4$s',
						esc_attr( $permalink ),
						esc_attr( $title ),
						$encoded_space,
				...
				case 'pinterest' :
					$link = sprintf( 'http://www.pinterest.com/pin/create/button/?url=%1$s&media=%2$s&description=%3$s',
						esc_attr( $permalink ),
						urlencode( $media_url ),
						esc_attr( $title )
					);
	...
	function get_shares_count( $network = '', $share_counts_num = 0, $return_div = true, $post_id = '', $url = '', $is_ajax_request = true ) {
		...
		if ( $share_counts >= $share_counts_num ) {
			$share_counts_output = false == $return_div ? $share_counts : sprintf( '<div class="et_social_count"><span>%1$s</span></div>', $this->get_compact_number( (int) $share_counts ) );
		}
			...
			if ( ! is_wp_error( $theme_request ) && wp_remote_retrieve_response_code( $theme_request ) == 200 ){
				...
				if ( ! empty( $theme_response ) ) {
					...
					if ( 'googleplus' === $social_network ) {
						preg_match( '/<div id="aggregateCount" class="Oy">(\d+)<\/div>/', $theme_response, $matches );
	...
	function get_icons_list( $type, $media_url = '', $is_mobile_sidebar = false ) {
		$monarch_options = $this->monarch_options;
		...
		if ( ! isset( $monarch_options[ 'sharing_networks_networks_sorting' ] ) || empty( $monarch_options[ 'sharing_networks_networks_sorting' ] ) ) {
			...
			foreach ( $monarch_options[ 'sharing_networks_networks_sorting' ][ 'class' ] as $icon ) {
				$icon_name = $monarch_options[ 'sharing_networks_networks_sorting' ][ 'label' ][ $i ];
				$social_type = 'like' == $icon ? 'like' : 'share';
				$is_counts_cached = $this->check_cached_counts( get_the_ID(), $icon, 'share' );
				...
				$min_counts = true == $monarch_options[ 'sharing_' . $type . '_counts' ] ? sprintf( 'data-min_count="%1$s"', $monarch_options[ 'sharing_' . $type . '_counts_num' ] ) : '';
	...
	function generate_media_icons( $media_url = '' ) {
		...
		$media_icons = sprintf(
			...
			$this->get_icons_list( 'media', $media_url ),
	...
	function display_sidebar() {
		...
		if ( $this->check_applicability( $monarch_options[ 'sharing_sidebar_post_types' ], 'sidebar' ) ) {
			printf(
				'<div class="et_social_sidebar_networks et_social_visible_sidebar et_social_%8$s et_social_%6$s et_social_sidebar_%1$s%2$s%3$s%7$s%9$s">
					%4$s
					%5$s
			...
			if ( true != $monarch_options[ 'sharing_sidebar_mobile' ] ) {
				printf(
					...
					$this->get_icons_list( 'sidebar', '', true )
				);
			}
		}
	...
	function display_flyin() {
		...
		if ( $this->check_applicability( $monarch_options['sharing_flyin_post_types'], 'flyin' ) ) {
			printf(
				'<div class="et_social_flyin et_social_resize et_social_flyin_%1$s et_social_%8$s%13$s%15$s%16$s%18$s"%14$s%17$s>
				...
				true == $monarch_options[ 'sharing_flyin_trigger_leave' ] ? ' et_social_ouibounce_modal' : '',
	...
	function display_popup() {
		...
		if ( $this->check_applicability( $monarch_options['sharing_popup_post_types'], 'popup' ) ) {
			printf(
				'<div class="et_social_popup%12$s%14$s%15$s%17$s"%13$s%16$s>
				...
				'' !== $monarch_options[ 'sharing_popup_title_text' ] || '' !== $monarch_options[ 'sharing_popup_message_text' ]
					? sprintf( '<h3>%1$s</h3>', esc_html( stripslashes( $monarch_options[ 'sharing_popup_title_text' ] ) ) )
					: '',
				'' !== $monarch_options[ 'sharing_popup_message_text' ]
					? sprintf( '<p>%1$s</p>', esc_html( stripslashes( $monarch_options[ 'sharing_popup_message_text' ] ) ) )
					: '',
				'auto' == $monarch_options[ 'sharing_popup_col_number' ] ? 'autowidth' : esc_html( $monarch_options[ 'sharing_popup_col_number' ] . 'col' ),
				esc_attr( $monarch_options[ 'sharing_popup_icon_style' ] ),
				esc_attr( $monarch_options[ 'sharing_popup_icon_shape' ] ), //#5
				esc_attr( $monarch_options[ 'sharing_popup_icons_alignment' ] ),
				true == $monarch_options[ 'sharing_popup_counts' ] ? ' et_social_withcounts' : '',
				true == $monarch_options[ 'sharing_popup_total' ] ? ' et_social_withtotalcount' : '',
				esc_attr( $monarch_options[ 'sharing_popup_animation' ] ),
				true == $monarch_options[ 'sharing_popup_total' ]
					? sprintf(
						'<div class="et_social_totalcount et_social_dark">
							<span class="et_social_totalcount_count et_social_total_share" data-post_id="%2$s"></span>
							<span class="et_social_totalcount_label">%1$s</span>
					...
					: '', //#10
				$this->get_icons_list( 'popup' ),
				true == $monarch_options[ 'sharing_popup_auto_popup' ] ? ' et_social_auto_popup' : '',
				true == $monarch_options[ 'sharing_popup_auto_popup' ]
					? sprintf( ' data-delay="%1$s"', esc_attr( $monarch_options[ 'sharing_popup_popup_delay' ] ) )
					: '',
				true == $monarch_options[ 'sharing_popup_trigger_bottom' ] ? ' et_social_trigger_bottom' : '',
				true == $monarch_options[ 'sharing_popup_trigger_leave' ] ? ' et_social_ouibounce_modal' : '', //#15
				true == $monarch_options[ 'sharing_popup_cookies' ] ? ' data-cookie_duration="' . esc_attr( $monarch_options[ 'sharing_popup_cookie_duration' ] ) . '"' : '',
				true == $monarch_options[ 'sharing_popup_mobile' ] ? ' et_social_mobile_off' : ' et_social_mobile_on',
				true == $monarch_options[ 'sharing_popup_network_names' ] ? ' et_social_withnetworknames' : '',
				true == $monarch_options[ 'sharing_popup_spacing' ] ? ' et_social_nospace' : '' //#19
			);
		}
	}
	...
	function load_scripts_styles() {
		...
		wp_enqueue_script( 'et_monarch-ouibounce', ET_MONARCH_PLUGIN_URI . '/js/ouibounce.js', array( 'jquery' ), $this->plugin_version, true );
